...
def _product_profile_payload(value: ProductCapabilities) -> dict[str, object]:
    return {
        ...
        "venue": value.venue,
        "market_hours_model": value.market_hours_model.value,
        "allowed_order_types": sorted(item.value for item in value.allowed_order_types),
        "allowed_time_in_force": sorted(item.value for item in value.allowed_time_in_force),
        "fractionable": value.fractionable,
        "marginable": value.marginable,
        "shortable": value.shortable,
        "protection_model": value.protection_model.value,
        "source": value.source,
