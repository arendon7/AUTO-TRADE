...
class SQLitePaperSubmissionRegistry:
    ...
    This registry deliberately performs no network I/O and exposes no POST API.
    A binding is frozen before external submission. `mark_submit_attempt_unknown`
    must be persisted *before* a future network write; once marked, the order is
    UNKNOWN and cannot be submitted again. Recovery is lookup/reconciliation by
    deterministic `client_order_id`, never a blind POST retry.

    Each per-order event stream is append-only SHA-256 chained. A separately
    hash-protected control row anchors the current event head, so mutation,
    reordering, sequence gaps and tail deletion are all detectable on every read.
    """
        ...
        try:
            conn.executescript(
                ...
                CREATE TABLE IF NOT EXISTS alpaca_paper_submission_control (
                    ...
                    absence_observation_count INTEGER NOT NULL,
                ...
                CREATE INDEX IF NOT EXISTS idx_alpaca_paper_submission_events_order
                ON alpaca_paper_submission_events(order_id, sequence);
                """
            )
        finally:
            conn.close()
    ...
    def prepare(self, binding: PaperSubmissionBinding) -> PaperSubmissionState:
        ...
        try:
            ...
            by_order = conn.execute(
                """
                SELECT order_id, client_order_id, binding_json, binding_hash
                FROM alpaca_paper_submission_bindings WHERE order_id = ?
                """,
                (binding.order_id,),
            ).fetchone()
            by_client = conn.execute(
                """
                SELECT order_id, client_order_id, binding_json, binding_hash
                FROM alpaca_paper_submission_bindings WHERE client_order_id = ?
                """,
            ...
            if by_order is not None or by_client is not None:
                if by_order is None or by_client is None:
                    raise PaperSubmissionIntegrityError(
                        "submission binding indexes disagree"
                    )
                existing = _binding_from_row(by_order)
                if by_client["order_id"] != existing.order_id:
                ...
                if existing.fingerprint != binding.fingerprint:
                    raise PaperSubmissionConflict(
                        "local order/client_order_id is already bound to different payload"
                    )
            ...
            binding_json = _canonical_json(_binding_payload(binding))
            conn.execute(
                """
    ...
    def mark_submit_attempt_unknown(
        ...
        *,
        ...
        try:
            ...
            duplicate = _event_by_payload_id(
                events,
            ...
            if state.status is not PaperSubmissionStatus.PREPARED:
                raise PaperSubmissionBlocked(
                    f"external PAPER submit is blocked from {state.status.value}; reconcile first"
                )
            event = _make_event(
                order_id=order_id,
        ...
        *,
        ...
        try:
            ...
            if broker_order_payload_hash != binding.order_payload_hash:
                raise PaperSubmissionConflict(
                    "reconciled broker order payload does not match frozen binding"
                )
            ...
            if state.status is PaperSubmissionStatus.ACKNOWLEDGED:
                if (
                    ...
                    return state
                raise PaperSubmissionConflict(
                    "submission is already acknowledged to a different broker order"
                )
            if state.status is not PaperSubmissionStatus.UNKNOWN:
                raise PaperSubmissionBlocked(
                    "broker acknowledgement is accepted only after an UNKNOWN submit attempt"
                )
            _reject_reused_request_id(events, request_id)
    ...
    def _verify_locked(
        ...
        rows = conn.execute(
            """
            SELECT order_id, sequence, event_type, occurred_at, payload_json,
                   previous_event_hash, event_hash
            FROM alpaca_paper_submission_events
        ...
        control_row = conn.execute(
            """
            SELECT order_id, binding_hash, status, event_sequence, event_head_hash,
                   attempt_count, absence_observation_count, broker_order_id,
                   broker_client_order_id, updated_at, control_hash
            FROM alpaca_paper_submission_control WHERE order_id = ?
            """,
            (order_id,),
        ).fetchone()
...
def deterministic_client_order_id(order: OrderRecord) -> str:
    ...
    # <= 128 Alpaca characters, intentionally opaque and strategy-independent.
...
def _event_from_row(row: sqlite3.Row) -> PaperSubmissionEvent:
    ...
    if row["occurred_at"] != _iso(event.occurred_at):
        raise PaperSubmissionIntegrityError("submission event timestamp is not canonical")
    if row["payload_json"] != _canonical_json(event.payload):
    ...
    for event in events:
        ...
        if event.event_type is PaperSubmissionEventType.PREPARED:
            if event.sequence != 1 or status is not None:
                raise PaperSubmissionIntegrityError("PREPARED must be the first submission event")
            if set(event.payload) != {"binding_hash"}:
                raise PaperSubmissionIntegrityError("PREPARED event payload is non-canonical")
            status = PaperSubmissionStatus.PREPARED
            ...
            if status is not PaperSubmissionStatus.PREPARED or attempt_count != 0:
                raise PaperSubmissionIntegrityError(
                    "submission attempt event has invalid predecessor state"
                )
            attempt_id = _required_str(event.payload, "attempt_id")
            _validate_id(attempt_id, "attempt_id")
            if attempt_id in seen_attempt_ids:
                raise PaperSubmissionIntegrityError("duplicate submission attempt_id in event chain")
            seen_attempt_ids.add(attempt_id)
            if event.payload.get("client_order_id") != binding.client_order_id:
                raise PaperSubmissionIntegrityError(
                    "submission attempt client_order_id mismatch"
                )
            if set(event.payload) != {
                ...
                raise PaperSubmissionIntegrityError(
                    "submission attempt payload is non-canonical"
                )
            attempt_count = 1
            status = PaperSubmissionStatus.UNKNOWN
            ...
            if event.payload.get("lookup") != "client_order_id":
                raise PaperSubmissionIntegrityError(
                    "absence reconciliation lookup type is invalid"
                )
            if set(event.payload) != {
                "binding_hash",
                "client_order_id",
            ...
            if status is not PaperSubmissionStatus.UNKNOWN:
                raise PaperSubmissionIntegrityError(
                    "acknowledgement reconciliation has invalid predecessor state"
                )
            request_id = _required_str(event.payload, "request_id")
            _validate_request_id(request_id)
            if request_id in seen_request_ids:
            ...
            if candidate_client_id != binding.client_order_id:
                raise PaperSubmissionIntegrityError(
                    "acknowledged broker client_order_id mismatch"
                )
            ...
            if candidate_payload_hash != binding.order_payload_hash:
                raise PaperSubmissionIntegrityError(
                    "acknowledged broker order payload hash mismatch"
                )
            if set(event.payload) != {
                "binding_hash",
                ...
                raise PaperSubmissionIntegrityError(
                    "acknowledgement reconciliation payload is non-canonical"
                )
            broker_order_id = candidate_order_id
            broker_client_order_id = candidate_client_id
            status = PaperSubmissionStatus.ACKNOWLEDGED
        else:
            raise PaperSubmissionIntegrityError("unsupported submission event type")
...
def _insert_control(conn: sqlite3.Connection, state: PaperSubmissionState) -> None:
    conn.execute(
        ...
        INSERT INTO alpaca_paper_submission_control(
            order_id, binding_hash, status, event_sequence, event_head_hash,
            attempt_count, absence_observation_count, broker_order_id,
            broker_client_order_id, updated_at, control_hash
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
...
def _update_control(conn: sqlite3.Connection, state: PaperSubmissionState) -> None:
    cursor = conn.execute(
        ...
        SET binding_hash = ?, status = ?, event_sequence = ?, event_head_hash = ?,
            attempt_count = ?, absence_observation_count = ?, broker_order_id = ?,
            broker_client_order_id = ?, updated_at = ?, control_hash = ?
        WHERE order_id = ?
        """,
...
def _control_from_row(row: sqlite3.Row) -> PaperSubmissionState:
    ...
    # client_order_id is stored in the immutable binding; it is injected by the
    # caller after parsing, before hash comparison.
    return state


def _state_with_client_id(
    raw: PaperSubmissionState, client_order_id: str
) -> PaperSubmissionState:
    state = _make_state(
        ...
        event_head_hash=raw.event_head_hash,
        attempt_count=raw.attempt_count,
        absence_observation_count=raw.absence_observation_count,
        broker_order_id=raw.broker_order_id,
        broker_client_order_id=raw.broker_client_order_id,
        updated_at=raw.updated_at,
    )
    if state.control_hash != raw.control_hash:
