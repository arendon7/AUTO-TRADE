...
class FinalGuardedCryptoEntryTransport:
    """PRE_IO interlock between durable UNKNOWN and the delegated transport.

    The dedicated writer remains the only direct network authority. It first
    persists ENTRY_SUBMISSION_UNKNOWN and then calls this transport. This layer
    requires a valid Final Freshness PRE_IO attestation for the exact
    client_order_id before delegating. It performs no network I/O itself and
    ...
    def post(
        ...
        *,
        ...
        if not isinstance(attestation, CryptoFinalWriteAttestation):
            raise CryptoPreIoInterlockError("PRE_IO authorizer returned invalid evidence")
        if attestation.phase is not CryptoFinalWritePhase.PRE_IO:
            raise CryptoPreIoInterlockError("guarded crypto transport requires PRE_IO attestation")
        if attestation.lifecycle_status is not CryptoLifecycleStatus.ENTRY_SUBMISSION_UNKNOWN:
            raise CryptoPreIoInterlockError("PRE_IO attestation must observe durable ENTRY_SUBMISSION_UNKNOWN")
        if attestation.entry_attempt_count != 1:
            raise CryptoPreIoInterlockError("PRE_IO attestation must observe exactly one entry attempt")
        if attestation.client_order_id != client_order_id:
            raise CryptoPreIoInterlockError("PRE_IO attestation client_order_id differs from request")
