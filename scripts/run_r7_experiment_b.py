...
from autotrade.research.experiment_b import build_experiment_b_program
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run preregistered R7 Experiment B against bounded Binance Spot public "
            "klines. DEVELOPMENT only; protected HOLDOUT is never checked out."
        )
    )
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("artifacts/r7-b"))
...
def main() -> int:
    ...
    # Experiment B deliberately does not call split.protected_holdout.checkout().
    ...
    program = build_experiment_b_program(quantity=args.research_quantity)
    ...
    result = DevelopmentResearchAutopilot(ledger=ledger).run(
        ...
        program=program,  # runtime protocol intentionally mirrors StrategyProgram
