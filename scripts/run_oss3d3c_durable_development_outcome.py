...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Reconstruct and persist OSS-3D3C durable DEVELOPMENT outcome evidence "
            "after proving stable scientific equality with the certified D3A baseline"
        )
    )
    parser.add_argument("--certified-d3a-baseline", required=True)
    ...
    bundle = reconstruct_and_write_durable_development_outcome(
        certified_d3a_baseline_path=args.certified_d3a_baseline,
    ...
    print(
        ...
        bundle.certified_d3a_baseline_fingerprint,
    )
    print("source_d3a_evidence_fingerprint:", bundle.source_d3a_evidence_fingerprint)
    print("scientific_outcome_fingerprint:", bundle.scientific_outcome_fingerprint)
    print("scientific_identity_policy:", bundle.scientific_identity_policy)
