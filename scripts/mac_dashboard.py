...
def _command(action: str, payload: dict[str, object]) -> tuple[list[str], tuple[str, str] | None]:
    ...
    workspace = _workspace(payload, allow_missing=action == "init_workspace")
    ...
    if action == "doctor":
        return base + ["doctor", "--workspace", workspace], None
    if action == "rehearsal":
    ...
    if action == "safety_rehearsal":
        ...
        limit_price = str(payload.get("limit_price") or "100").strip()
        return base + [
            "safety-rehearsal",
            "--symbol", symbol,
            "--quantity", quantity,
...
def _build_meta() -> dict[str, object]:
    info: dict[str, str] = {}
    build_info = ROOT / "MAC_BUILD_INFO.txt"
    if build_info.is_file():
        for line in build_info.read_text(encoding="utf-8").splitlines():
            if "=" in line:
                key, value = line.split("=", 1)
                info[key.strip()] = value.strip()
    install_receipt = ROOT / "MAC_FULL_INSTALL_RECEIPT.json"
    ...
    if install_receipt.is_file():
        try:
            ...
            receipt = None
    return {
        "source_head": info.get("source_head") or (receipt or {}).get("source_head") or "UNKNOWN",
        "bundle_mode": "FULL_OFFLINE" if (ROOT / "runtime").is_dir() else "SOURCE_BOOTSTRAP",
...
class DashboardHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if parsed.path == "/":
            ...
            html = html.replace("__CSRF_TOKEN__", self.dashboard_server.csrf_token)
