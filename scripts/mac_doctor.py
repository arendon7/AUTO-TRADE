...
def main(argv: list[str] | None = None) -> int:
    ...
    provenance_ok = branch == EXPECTED_BRANCH or packaged
    ...
    report: dict[str, object] = {
        ...
        "branch_matches_r6": provenance_ok,
        "provenance_mode": "CERTIFIED_PACKAGE" if packaged else "GIT_BRANCH",
    ...
    if not provenance_ok:
        report["provenance_warning"] = (
            "Expected the R6 integration branch or a packaged MAC_BUILD_INFO.txt with a valid source_head"
        )
        ok = False
    if write_enabled:
