...
def test_guard_blocks_wrong_order_identity_role_or_expired_window(tmp_path) -> None:
    ctx = _setup(tmp_path / "role")
    wrong_role = replace(ctx.broker_order, role=CryptoOrderRole.PROTECTION)
    with pytest.raises(CryptoColdStartFinalWriteBlocked, match="ENTRY only"):
        _pre(ctx, broker_order=wrong_role)
