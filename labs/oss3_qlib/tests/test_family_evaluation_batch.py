...
from labs.oss3_qlib.family_evaluation_batch import (
    ...
    FamilyEvaluationBatchCompatibilityError,
...
def _train_features():
    ...
    for day in range(8):
        ...
        for idx, symbol in enumerate(SYMBOLS, start=1):
            x1 = 0.8 + day * 0.55 + idx * 0.19
            x2 = 1.3 - day * 0.07 + idx * 0.04 + ((day + idx) % 3) * 0.03
            rows.append(
                FactorMatrixRow(
...
def _frozen_family(tmp_path: Path, monkeypatch):
    ...
    for binding in request_set.bindings:
        ...
        run_evidence = run_isolated_qlib_family_candidate(
            ...
            runtime_identity_output_path=runtime_path,
            run_evidence_output_path=evidence_path,
        )
    ...
    return plan, request_set, tuple(outputs), dev_labels
...
def test_end_to_end_six_real_models_preregister_then_evaluate_and_tournament(tmp_path, monkeypatch):
    plan, request_set, outputs, labels = _frozen_family(tmp_path, monkeypatch)
    ...
    assert evidence.tournament_evidence.runtime_environment_hash == outputs[0].runtime_environment.fingerprint
...
def test_prepare_is_deterministic_and_binds_label_artifact_without_metrics(tmp_path, monkeypatch):
    plan, request_set, outputs, labels = _frozen_family(tmp_path, monkeypatch)
...
def test_evaluation_requires_durable_preregistration(tmp_path, monkeypatch):
    plan, request_set, outputs, labels = _frozen_family(tmp_path, monkeypatch)
    prereg = _preregistration(plan, request_set, outputs, labels)
    ledger = SQLiteTrialLedger(tmp_path / "missing.sqlite")
...
def test_missing_candidate_fails_before_preregistration(tmp_path, monkeypatch):
    plan, request_set, outputs, labels = _frozen_family(tmp_path, monkeypatch)
...
def test_output_substitution_after_preregistration_fails(tmp_path, monkeypatch):
    plan, request_set, outputs, labels = _frozen_family(tmp_path, monkeypatch)
    prereg = _preregistration(plan, request_set, outputs, labels)
    ledger = SQLiteTrialLedger(tmp_path / "trials.sqlite")
    preregister_family_evaluation(ledger, prereg, now=datetime(2026, 3, 20, tzinfo=UTC))
    tampered = (replace(outputs[0], candidate_id=outputs[1].candidate_id),) + outputs[1:]
...
def test_different_development_label_artifact_after_preregistration_fails(tmp_path, monkeypatch):
    plan, request_set, outputs, labels = _frozen_family(tmp_path, monkeypatch)
    prereg = _preregistration(plan, request_set, outputs, labels)
    ledger = SQLiteTrialLedger(tmp_path / "trials.sqlite")
    preregister_family_evaluation(ledger, prereg, now=datetime(2026, 3, 20, tzinfo=UTC))
    altered = _development_labels(_development_features(), value_shift=0.001)
...
def test_train_labels_are_rejected_as_evaluation_labels(tmp_path, monkeypatch):
    plan, request_set, outputs, _ = _frozen_family(tmp_path, monkeypatch)
    train_features = _train_features()
    train_labels = _train_labels(train_features)
...
def test_evaluating_twice_is_rejected_by_untouched_ledger_rule(tmp_path, monkeypatch):
    plan, request_set, outputs, labels = _frozen_family(tmp_path, monkeypatch)
    prereg = _preregistration(plan, request_set, outputs, labels)
    ledger = SQLiteTrialLedger(tmp_path / "trials.sqlite")
    now = datetime(2026, 3, 20, tzinfo=UTC)
...
def test_preregistration_and_completed_evidence_are_authority_denied(tmp_path, monkeypatch):
    plan, request_set, outputs, labels = _frozen_family(tmp_path, monkeypatch)
    prereg = _preregistration(plan, request_set, outputs, labels)
