...
it.  It creates a portable, canonical, hash-bound handoff for the next research
frontier without reranking candidates, recomputing metrics, observing
FINAL_HOLDOUT or granting any execution authority.

The bundle is intentionally evidence-only.  It does not reconstruct or invoke
Qlib, market-data acquisition, broker, OMS, Safety, PAPER or LIVE surfaces.
"""
...
class DurableDevelopmentOutcomeBundle:
    ...
    def __post_init__(self) -> None:
        ...
        if self.next_frontier != NEXT_FRONTIER:
            raise DurableDevelopmentOutcomeGovernanceError("D3C next frontier must remain D2J preregistration only")
        for name in (
        ...
        d3a = _parse_canonical_document(self.source_d3a_evidence_json, "source D3A evidence")
        d2h = _parse_canonical_document(self.d2h_preregistration_json, "D2H preregistration")
        batch = _parse_canonical_document(self.d2h_batch_evidence_json, "D2H batch evidence")
        winner = _parse_canonical_document(self.d2i_winner_seal_json, "D2I winner seal")

        if _hash_payload(d3a) != self.source_d3a_evidence_fingerprint:
            raise DurableDevelopmentOutcomeIntegrityError("D3C source D3A evidence fingerprint mismatch")
        if _hash_payload(d2h) != self.d2h_preregistration_fingerprint:
            raise DurableDevelopmentOutcomeIntegrityError("D3C D2H preregistration fingerprint mismatch")
        if _hash_payload(batch) != self.d2h_batch_evidence_fingerprint:
            raise DurableDevelopmentOutcomeIntegrityError("D3C D2H batch fingerprint mismatch")
        if _hash_payload(winner) != self.d2i_winner_seal_fingerprint:
            raise DurableDevelopmentOutcomeIntegrityError("D3C D2I winner fingerprint mismatch")
    ...
    def source_d3a_evidence(self) -> dict[str, object]:
        return _parse_canonical_document(self.source_d3a_evidence_json, "source D3A evidence")

    @property
    def d2h_preregistration(self) -> dict[str, object]:
        return _parse_canonical_document(self.d2h_preregistration_json, "D2H preregistration")

    @property
    def d2h_batch_evidence(self) -> dict[str, object]:
        return _parse_canonical_document(self.d2h_batch_evidence_json, "D2H batch evidence")

    @property
    def d2i_winner_seal(self) -> dict[str, object]:
        return _parse_canonical_document(self.d2i_winner_seal_json, "D2I winner seal")

    @property
    def selected_trial_id(self) -> str:
        value = self.d2i_winner_seal.get("selected_trial_id")
        if not isinstance(value, str) or not value:
            raise DurableDevelopmentOutcomeIntegrityError("D3C winner lacks selected_trial_id")
        return value
...
def build_durable_development_outcome(
    *,
    ...
    """Freeze completed D3A -> D2H -> D2I evidence without recomputation."""
    ...
    try:
        ...
        raise DurableDevelopmentOutcomeIntegrityError("D3C D2I winner does not rebind to supplied D2H evidence") from exc

    to_dict = getattr(source_d3a_evidence, "to_dict", None)
    fingerprint = getattr(source_d3a_evidence, "fingerprint", None)
    if not callable(to_dict) or not isinstance(fingerprint, str):
        raise TypeError("source_d3a_evidence must expose to_dict() and fingerprint")
    d3a_payload = to_dict()
    if not isinstance(d3a_payload, Mapping):
        raise TypeError("source_d3a_evidence.to_dict() must return a mapping")
    d3a_payload = dict(d3a_payload)
    if _hash_payload(d3a_payload) != fingerprint:
        raise DurableDevelopmentOutcomeIntegrityError("D3C source D3A object fingerprint is inconsistent")

    return DurableDevelopmentOutcomeBundle(
        contract_version=OSS3D3C_CONTRACT_VERSION,
        next_frontier=NEXT_FRONTIER,
        source_d3a_evidence_fingerprint=fingerprint,
        ...
        source_d3a_evidence_json=_canonical_json(d3a_payload),
...
def write_durable_development_outcome(bundle: DurableDevelopmentOutcomeBundle, path: str | Path) -> None:
    ...
    if len(raw) > MAX_BUNDLE_BYTES:
        raise DurableDevelopmentOutcomeGovernanceError("D3C serialized bundle exceeds size bound")
    staging = target.with_name(target.name + ".tmp")
    staging.write_bytes(raw)
    staging.replace(target)


def read_durable_development_outcome(path: str | Path) -> DurableDevelopmentOutcomeBundle:
    ...
    try:
        ...
        raise DurableDevelopmentOutcomeIntegrityError("D3C artifact is not UTF-8") from exc
    document = _strict_json_loads(text)
    if not isinstance(document, dict):
        raise DurableDevelopmentOutcomeIntegrityError("D3C top level must be an object")
    expected_keys = {
        "contract_version",
        "next_frontier",
        "source_d3a_evidence_fingerprint",
    ...
    if set(document) != expected_keys:
        raise DurableDevelopmentOutcomeIntegrityError("D3C top-level schema drifted")
    artifact_hash = document.pop("artifact_hash")
    _require_hash(artifact_hash, "artifact_hash")

    for name in ("source_d3a_evidence", "d2h_preregistration", "d2h_batch_evidence", "d2i_winner_seal"):
        if not isinstance(document[name], dict):
            raise DurableDevelopmentOutcomeIntegrityError(f"D3C {name} must be an object")

    bundle = DurableDevelopmentOutcomeBundle(
        contract_version=document["contract_version"],
        next_frontier=document["next_frontier"],
        source_d3a_evidence_fingerprint=document["source_d3a_evidence_fingerprint"],
    ...
    if bundle.fingerprint != artifact_hash:
        raise DurableDevelopmentOutcomeIntegrityError("D3C artifact hash mismatch")
    return bundle
...
def _verify_payload_links(
    *,
    ...
    if d3a.get("evidence_version") != SOURCE_D3A_EVIDENCE_VERSION or d3a.get("status") != SOURCE_D3A_COMPLETED_STATUS:
        raise DurableDevelopmentOutcomeGovernanceError("D3C requires a completed canonical D3A campaign")
    if d2h.get("preregistration_version") != OSS3D2H_PREREGISTRATION_VERSION:
        raise DurableDevelopmentOutcomeIntegrityError("D3C noncanonical D2H preregistration")
    if batch.get("evidence_version") != OSS3D2H_BATCH_EVIDENCE_VERSION:
        raise DurableDevelopmentOutcomeIntegrityError("D3C noncanonical D2H batch evidence")
    if winner.get("contract_version") != OSS3D2I_CONTRACT_VERSION or winner.get("next_frontier") != NEXT_FRONTIER:
        raise DurableDevelopmentOutcomeIntegrityError("D3C noncanonical D2I winner seal")

    for actual, expected, message in (
        (d3a.get("d2h_preregistration_fingerprint"), preregistration_fingerprint, "D3A/D2H preregistration"),
        (d3a.get("d2h_batch_evidence_fingerprint"), batch_fingerprint, "D3A/D2H batch"),
        (d3a.get("d2i_winner_seal_fingerprint"), winner_fingerprint, "D3A/D2I winner"),
        (winner.get("preregistration_fingerprint"), preregistration_fingerprint, "D2I/D2H preregistration"),
        (winner.get("d2h_batch_evidence_fingerprint"), batch_fingerprint, "D2I/D2H batch"),
        (batch.get("preregistration_fingerprint"), preregistration_fingerprint, "batch/D2H preregistration"),
    ):
        if actual != expected:
            raise DurableDevelopmentOutcomeIntegrityError(f"D3C cross-link mismatch: {message}")

    if d3a.get("d2f_plan_fingerprint") != d2h.get("d2f_plan_fingerprint"):
        raise DurableDevelopmentOutcomeIntegrityError("D3C D3A/D2H D2F plan mismatch")
    if d3a.get("d2f_request_set_fingerprint") != d2h.get("d2f_request_set_fingerprint"):
        raise DurableDevelopmentOutcomeIntegrityError("D3C D3A/D2H request-set mismatch")
    if d3a.get("development_label_artifact_hash") != d2h.get("development_label_artifact_hash"):
        raise DurableDevelopmentOutcomeIntegrityError("D3C DEVELOPMENT label artifact mismatch")
    if batch.get("development_label_artifact_hash") != d2h.get("development_label_artifact_hash"):
        raise DurableDevelopmentOutcomeIntegrityError("D3C D2H/batch label artifact mismatch")
    if batch.get("d2e_plan_fingerprint") != d2h.get("d2e_plan_fingerprint"):
        raise DurableDevelopmentOutcomeIntegrityError("D3C D2E plan fingerprint mismatch")
    if batch.get("tournament_evidence_hash") != d3a.get("d2e_tournament_evidence_fingerprint"):
        raise DurableDevelopmentOutcomeIntegrityError("D3C D3A/batch tournament mismatch")
    if winner.get("d2e_tournament_evidence_fingerprint") != batch.get("tournament_evidence_hash"):
        raise DurableDevelopmentOutcomeIntegrityError("D3C winner/batch tournament mismatch")
    if winner.get("shared_runner_code_hash") != batch.get("shared_runner_code_hash"):
        raise DurableDevelopmentOutcomeIntegrityError("D3C winner/batch runner mismatch")
    if winner.get("runtime_environment_hash") != batch.get("runtime_environment_hash"):
        raise DurableDevelopmentOutcomeIntegrityError("D3C winner/batch runtime mismatch")

    selected = winner.get("selected_trial_id")
    if not isinstance(selected, str) or not selected:
        raise DurableDevelopmentOutcomeIntegrityError("D3C winner selected_trial_id missing")
    if d3a.get("selected_trial_id") != selected:
        raise DurableDevelopmentOutcomeIntegrityError("D3C D3A/D2I selected winner mismatch")
    for d3a_name, winner_name in (
        ...
        if d3a.get(d3a_name) != winner.get(winner_name):
            raise DurableDevelopmentOutcomeIntegrityError(f"D3C D3A/D2I metric mismatch: {d3a_name}")

    expected_ids = tuple(candidate.candidate_id for candidate in CANONICAL_CANDIDATES)
    outputs = d3a.get("candidate_output_hashes")
    if not isinstance(outputs, list) or tuple(item[0] for item in outputs if isinstance(item, list) and len(item) == 2) != expected_ids:
        raise DurableDevelopmentOutcomeIntegrityError("D3C D3A output family differs from exact D2F six")
    bindings = d2h.get("candidate_output_bindings")
    if not isinstance(bindings, list) or tuple(item.get("candidate_id") for item in bindings if isinstance(item, dict)) != expected_ids:
        raise DurableDevelopmentOutcomeIntegrityError("D3C D2H binding family differs from exact D2F six")

    selected_bindings = [item for item in bindings if isinstance(item, dict) and item.get("candidate_id") == selected]
    evaluations = batch.get("evaluations")
    if not isinstance(evaluations, list):
        raise DurableDevelopmentOutcomeIntegrityError("D3C D2H batch evaluations missing")
    selected_evaluations = [item for item in evaluations if isinstance(item, dict) and item.get("candidate_id") == selected]
    if len(selected_bindings) != 1 or len(selected_evaluations) != 1:
        raise DurableDevelopmentOutcomeIntegrityError("D3C winner lacks unique D2H/batch lineage")
    ...
    for winner_name, expected in (
        ...
        ("d2d_evaluation_artifact_hash", evaluation.get("d2d_evaluation_artifact_hash")),
    ):
        if winner.get(winner_name) != expected:
            raise DurableDevelopmentOutcomeIntegrityError(f"D3C winner lineage mismatch: {winner_name}")

    if d2h.get("label_values_used") is not False or d2h.get("development_metrics_computed") is not False:
        raise DurableDevelopmentOutcomeGovernanceError("D3C requires pre-metric D2H preregistration semantics")
    if batch.get("label_values_used_after_preregistration") is not True or batch.get("development_metrics_computed") is not True:
        raise DurableDevelopmentOutcomeIntegrityError("D3C completed D2H batch semantics are incomplete")

    _deny_nested_authority(d3a, "D3A")
...
def _deny_nested_authority(document: Mapping[str, object], source: str) -> None:
    for key in (
        ...
        if document.get(key) is True:
            raise DurableDevelopmentOutcomeGovernanceError(f"D3C {source} escalates authority: {key}")
    if document.get("capital_authority", "NONE") != "NONE":
        raise DurableDevelopmentOutcomeGovernanceError(f"D3C {source} grants capital authority")
    if document.get("live_trading", "BLOCKED") != "BLOCKED":
        raise DurableDevelopmentOutcomeGovernanceError(f"D3C {source} grants LIVE authority")
...
def _deny_authority(bundle: DurableDevelopmentOutcomeBundle) -> None:
    if (
        ...
        raise DurableDevelopmentOutcomeGovernanceError("D3C cannot escalate research authority")
    if bundle.capital_authority != "NONE" or bundle.live_trading != "BLOCKED":
        raise DurableDevelopmentOutcomeGovernanceError("D3C cannot grant capital or LIVE")
...
def _parse_canonical_document(raw: str, name: str) -> dict[str, object]:
    ...
    if not isinstance(document, dict):
        raise DurableDevelopmentOutcomeIntegrityError(f"{name} must be an object")
    if _canonical_json(document) != raw:
        raise DurableDevelopmentOutcomeIntegrityError(f"{name} is not canonical JSON")
    return document
...
def _strict_json_loads(raw: str) -> object:
    def pairs_hook(pairs):
        ...
        for key, value in pairs:
            if key in result:
                raise DurableDevelopmentOutcomeIntegrityError("D3C JSON contains duplicate object key")
            result[key] = value
        return result
    ...
    try:
        ...
        raise DurableDevelopmentOutcomeIntegrityError("D3C artifact contains invalid JSON") from exc
...
def _require_hash(value: object, name: str) -> None:
    if not isinstance(value, str) or not _HASH_RE.fullmatch(value):
        raise DurableDevelopmentOutcomeIntegrityError(f"D3C invalid {name}")
