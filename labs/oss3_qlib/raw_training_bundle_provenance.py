"""OSS-3D2R raw-market provenance for the complete supervised TRAIN bundle.

D2R deterministically reconstructs OSS-3B TRAIN features, OSS-3C TRAIN labels
and the OSS-3D1 TrainingBundleArtifact from aligned raw OHLCV.

Three identities are intentionally separated:

* schema semantics: stable formula/source hashes shared across partitions;
* research-universe identity: instruments/venue/quote/timeframe, no bar bytes;
* material lineage: exact warmup/TRAIN dataset hashes and raw-source hash.

Feature rows are causal at t. Labels intentionally use exactly the next TRAIN
close, because supervised targets require future information; that access is
explicitly horizon-bound and cannot cross the TRAIN material. The terminal raw
TRAIN bar is used only as the last label endpoint, never as a sample origin.

Research only: no DEVELOPMENT, FINAL_HOLDOUT, economic holdout, predictions,
Qlib, network, broker, OMS, Safety, OrderIntent, PAPER, capital or LIVE.
"""
...
OSS3D2R_UNIVERSE_IDENTITY_POLICY = "OSS3D2R_RESEARCH_UNIVERSE_IDENTITY_V1"
...
def canonical_oss3d2r_feature_definitions() -> tuple[object, ...]:
    """Stable feature schema; exact raw bytes are bound in artifact manifests."""
...
def research_universe_identity_hash(universe: AlignedMarketUniverse) -> str:
    """Hash stable instrument semantics, deliberately excluding bar bytes."""
    if not isinstance(universe, AlignedMarketUniverse):
        raise TypeError("universe must be AlignedMarketUniverse")
    return _hash(
        {
            "policy": OSS3D2R_UNIVERSE_IDENTITY_POLICY,
            ...
            "instruments": [
                {
                    ...
                    "price_tick": str(dataset.instrument.price_tick),
                    "quantity_step": str(dataset.instrument.quantity_step),
                }
...
class RawTrainingMarketSource:
    ...
    def __post_init__(self) -> None:
        ...
        if research_universe_identity_hash(self.warmup_universe) != research_universe_identity_hash(self.training_universe):
            raise RawTrainingBundleProvenanceIntegrityError("warmup/TRAIN research universe identity differs")
    ...
    def universe_identity_hash(self) -> str:
        return research_universe_identity_hash(self.training_universe)
        ...
        return _hash(
            {
                ...
                "training_universe_material_hash": self.training_universe.universe_hash,
    ...
    def partition_end(self) -> datetime:
        # Half-open OSS-3C bound immediately after final label availability.
...
class OSS3RawTrainingBundleProvenanceReceipt:
    ...
    warmup_universe_material_hash: str
    warmup_dataset_set_hash: str
    training_universe_material_hash: str
    ...
    universe_semantics_separated_from_material_lineage: bool
    ...
    def __post_init__(self) -> None:
        ...
        for name in (
            "research_split_hash", "research_universe_identity_hash", "raw_training_source_hash",
            "warmup_universe_material_hash", "warmup_dataset_set_hash", "training_universe_material_hash",
            "training_dataset_set_hash", "feature_formula_registry_hash", "label_formula_hash",
            "label_definition_hash", "producer_semantic_hash", "feature_artifact_hash",
            "feature_row_payload_hash", "label_artifact_hash", "label_row_payload_hash",
            "training_bundle_hash", "training_bundle_manifest_hash", "receipt_hash",
        ):
            _require_hash(getattr(self, name), name)
        if self.feature_formula_hashes != tuple((f.name, f.formula_hash) for f in canonical_oss3d2q_formulas()):
        ...
        if (
            not self.exact_feature_label_keyset
            or not self.schema_semantics_separated_from_material_lineage
            or not self.universe_semantics_separated_from_material_lineage
        ):
            raise RawTrainingBundleProvenanceIntegrityError("D2R keyset/identity separation proof missing")
        if (
    ...
    def to_dict(self, *, include_hash: bool = True) -> dict[str, object]:
        payload: dict[str, object] = {
            ...
            "warmup_universe_material_hash": self.warmup_universe_material_hash,
            "warmup_dataset_set_hash": self.warmup_dataset_set_hash,
            "training_universe_material_hash": self.training_universe_material_hash,
            "training_dataset_set_hash": self.training_dataset_set_hash,
            ...
            "universe_semantics_separated_from_material_lineage": self.universe_semantics_separated_from_material_lineage,
...
def derive_raw_training_bundle(
    *, raw_source: RawTrainingMarketSource, campaign_id: str, research_split_hash: str,
    ...
    features, labels, bundle = _derive_artifacts(raw_source, campaign_id, research_split_hash)
    receipt = _build_receipt(raw_source, campaign_id, research_split_hash, features, labels, bundle)
...
def verify_raw_training_bundle_provenance(
    *, raw_source: RawTrainingMarketSource, campaign_id: str, research_split_hash: str,
    features: FactorMatrixArtifact, labels: SupervisedLabelArtifact,
    bundle: TrainingBundleArtifact, receipt: OSS3RawTrainingBundleProvenanceReceipt,
) -> None:
    ef, el, eb = _derive_artifacts(raw_source, campaign_id, research_split_hash)
    if features != ef:
    ...
    expected = _build_receipt(raw_source, campaign_id, research_split_hash, features, labels, bundle)
    if receipt != expected:
        raise RawTrainingBundleProvenanceIntegrityError("D2R receipt does not rebind to raw derivation")
...
def _derive_paired_rows(raw_source: RawTrainingMarketSource) -> tuple[tuple[FactorMatrixRow, ...], tuple[SupervisedLabelRow, ...]]:
    ...
    for origin_index in range(training.bar_count - 1):
        ...
        for symbol in training.symbols:
            history = _causal_history(raw_source, symbol, origin_index, origin_as_of)
            feature_rows.append(
                FactorMatrixRow(
                    as_of=origin_as_of.isoformat(),
                    available_at=origin_as_of.isoformat(),
                    symbol=symbol,
                    values=tuple(_feature_value(formula, history) for formula in formulas),
                )
            )
    ...
    fkeys = tuple((row.as_of, row.symbol) for row in feature_rows)
    lkeys = tuple((row.label_as_of, row.symbol) for row in label_rows)
    if fkeys != lkeys or len(feature_rows) != raw_source.sample_count:
        raise RawTrainingBundleProvenanceIntegrityError("D2R exact paired keyset/sample count drifted")
    return tuple(feature_rows), tuple(label_rows)


def _causal_history(raw_source: RawTrainingMarketSource, symbol: str, origin_index: int, as_of: datetime) -> tuple[Bar, ...]:
    all_seen = raw_source.warmup_universe.dataset(symbol).bars + raw_source.training_universe.dataset(symbol).bars[: origin_index + 1]
    selected = all_seen[-(CANONICAL_LOOKBACK_BARS + 1):]
    if len(selected) != CANONICAL_LOOKBACK_BARS + 1:
        raise RawTrainingBundleProvenanceGovernanceError("insufficient D2R feature history")
    ...
    if selected[-1].ended_at.astimezone(timezone.utc) != as_of:
        raise RawTrainingBundleProvenanceIntegrityError("feature history does not terminate at as_of")
    return selected


def _feature_value(formula: CanonicalFeatureFormula, history: tuple[Bar, ...]) -> float:
...
def _build_receipt(
    ...
    values: dict[str, object] = {
        ...
        "campaign_id": campaign_id,
        "research_split_hash": research_split_hash,
        "research_universe_identity_hash": raw_source.universe_identity_hash,
        "raw_training_source_hash": raw_source.source_hash,
        "warmup_universe_material_hash": raw_source.warmup_universe.universe_hash,
        "warmup_dataset_set_hash": raw_source.warmup_dataset_set_hash,
        "training_universe_material_hash": raw_source.training_universe.universe_hash,
        ...
        "universe_semantics_separated_from_material_lineage": True,
