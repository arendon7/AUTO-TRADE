...
if [[ "$(uname -s)" != "Darwin" ]]; then
  echo "AUTO-TRADE CRYPTO PAPER LAB: este launcher es exclusivamente para macOS." >&2
  exit 2
fi
if [[ "${R6_EXTERNAL_PAPER_WRITE:-DISABLED}" == "ENABLED" ]]; then
  echo "AUTO-TRADE CRYPTO PAPER LAB BLOCKED: R6_EXTERNAL_PAPER_WRITE=ENABLED." >&2
  exit 2
fi
...
exec "$ROOT/.venv/bin/python" "$ROOT/scripts/mac_crypto_dashboard.py"
